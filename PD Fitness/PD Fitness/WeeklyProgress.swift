...
class WeeklyProgress: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for j in 0 ... 6
        {
            ...
            let i = -j
            //calculates the day from i days ago
            dateComponent.day = i
            ...
            ref.child(completePath[j]).observeSingleEvent(of: .value, with: { snapshot in
                ...
                {
                    //if task count exists, then grabs the value into an array
                ...
                {
                    //sets task count to 0 if the child does not exist
    ...
    {
        let chartDataSet = BarChartDataSet(entries: Entries, label: "Completed Tasks")  //creating a dataset for completed tasks
        let chartDataSet1 = BarChartDataSet(entries: Entries2, label: "Total Tasks")    //creating a dataset for total tasks


        let dataSets: [BarChartDataSet] = [chartDataSet,chartDataSet1]      //puttng both datasets in an array so they can be graphed together
        ...
        BarChartView.setVisibleXRangeMaximum(2) //enables sliding on the chart view
        BarChartView.moveViewToX(0)             //sets the page to open on the left side
    ...
    func setChart(Date: [String])
    {
        ...
        BarChartView.animate(xAxisDuration: 2.5, yAxisDuration: 2.5, easingOption: .easeInQuart)        //animates the bars when they appear

        
        let xaxis = BarChartView.xAxis          //shorter reference to the x-axis
        xaxis.drawGridLinesEnabled = true
        xaxis.centerAxisLabelsEnabled = true
        xaxis.valueFormatter = IndexAxisValueFormatter(values:self.dateFormString)      //sets the x-axis names to the dates
        let yaxis = BarChartView.leftAxis       //shorter reference to the y-axis
        yaxis.spaceTop = 0.35
        yaxis.axisMinimum = 0
